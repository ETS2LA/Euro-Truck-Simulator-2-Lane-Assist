...
class _SubElement:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        _elements[self._name] = {"style": self.style.to_dict(), "children": dictionary}
        dictionary = self.previous
        ...
        self.previous.append(
            {
                "button": {
                    ...
                    "type": self.type,
                    "name": self.name,
                    "action": get_fully_qualified_name(self.action)
                    if self.action
                    else None,
                    "style": self.style.to_dict(),
        ...
        self.previous.append(
            {
                "tabs": {
                    "id": self.id,
                    "style": self.style.to_dict(),
                    "changed": get_fully_qualified_name(self.changed)
                    if self.changed
                    else None,
                    "children": dictionary,
                }
...
class Spinner:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        global dictionary
        self.previous.append({"spinner": {"id": self.id, "children": dictionary}})
        dictionary = self.previous
        ...
        dictionary.append(
            {
                "graph": {
                    ...
                    "config": self.config,
                    "x": self.x.to_dict(),
                    "y": [axis.to_dict() for axis in self.y]
...
class ButtonWithTitleDescription:
    ...
    def __init__(
        ...
        with Container(
            ...
            + styles.Classname(
                "border items-center border-muted rounded-md p-4 w-full justify-between bg-input/10"
            )
            + styles.Gap("10px")
            + self.style
        ):
...
class ComboboxWithTitleDescription:
    ...
    def __init__(
        ...
        with Container(
            style=styles.FlexHorizontal()
            + styles.Classname(
                "border justify-between rounded-md p-4 w-full bg-input/10"
            )
            + styles.Gap("10px")
            + self.style
        ):
    ...
    def __init__(
        ...
        with Container(
            ...
            + styles.Classname(
                ...
                + ("bg-input/30" if default else "bg-input/10")
            ),
...
class InputWithTitleDescription:
    ...
    def __init__(
        ...
        with Container(
            ...
            + styles.Padding("14px 16px 16px 16px")
            + styles.Classname(
                "border justify-between items-center rounded-md w-full bg-input/10"
            )
            + self.style
        ):
