...
def RestoreConsole():
    ...
    try:
        if variables.OS == "nt":
            if variables.CONSOLEHWND is not None and variables.CONSOLENAME is not None:
                win32gui.ShowWindow(variables.CONSOLEHWND, win32con.SW_RESTORE)
            else:
                try:
                    variables.CONSOLENAME = win32console.GetConsoleTitle()
                    variables.CONSOLEHWND = win32gui.FindWindow(
                        None, str(variables.CONSOLENAME)
                    )
                    win32gui.ShowWindow(variables.CONSOLEHWND, win32con.SW_RESTORE)
                    ...
                    print(
                        "\033[91m" + "Failed to restore console: " + "\033[0m" + str(e)
                    )
        else:
            print(
                "\033[91m"
                + "Failed to restore console: "
                + "\033[0m"
                + "Unsupported OS"
            )
    except Exception as e:
...
def HideConsole():
    ...
    try:
        if variables.OS == "nt":
            ...
            print(
                "\033[91m" + "Failed to hide console: " + "\033[0m" + "Unsupported OS"
            )
    except Exception as e:
...
def CloseConsole():
    ...
    try:
        if variables.OS == "nt":
            ...
            print(
                "\033[91m" + "Failed to close console: " + "\033[0m" + "Unsupported OS"
            )
    except Exception as e:
