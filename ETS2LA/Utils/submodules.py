...
def DownloadSubmoduleViaCDN(folder: str, cdn_url: str, cdn_path: str):
    try:
        print(
            f"{YELLOW} -- Downloading the submodule from ETS2LA servers due to no github connectivity -- {END}"
        )

        DownloadFile(f"{cdn_url}", "download.zip", chunk_size=8192, rich=True)

        # Extract the zip file
        with zipfile.ZipFile("download.zip", "r") as zip_ref:
            zip_ref.extractall("temp/extraction")
...
def EnsureSubmoduleExists(
    ...
    if not os.path.exists(folder):
        print(
            f"{GREEN} -- Please wait, we need to download the following submodule: {YELLOW} {folder} {GREEN} -- {END}"
        )
        try:
        ...
        try:
            if post_download_action != "":
                print(
                    f"{GREEN} -- Running post download action for submodule: {YELLOW} {folder} {GREEN} -- {END}"
                )
                ExecuteCommand(post_download_action)
        except Exception:
            print(
                f"{RED} -- Failed to run post download action for submodule: {YELLOW} {folder} {RED} -- {END}"
            )
        ...
        if did_update and post_update_action != "":
            print(
                f"{GREEN} -- Running post update action for submodule: {YELLOW} {folder} {GREEN} -- {END}"
            )
...
def CheckForSubmoduleUpdate(folder: str, cdn_url: str = "", cdn_path: str = ""):
    ...
    try:
        ...
        try:
            if time.time() - download_time > 86400:  # = 1 day
                print(
                    f"{GREEN} -- Please wait, we need to redownload the following submodule: {YELLOW} {folder} {GREEN} -- {END}"
                )
                DownloadSubmoduleViaCDN(folder, cdn_url, cdn_path)
                return True
        except Exception:
    ...
    try:
        ...
        if current_hash != origin_hash:
            print(
                f"{GREEN} -- Please wait, we need to update the following submodule: {YELLOW} {folder} {GREEN} -- {END}"
            )
            ExecuteCommand(f"git -C {folder} pull")
            return True
    except Exception:
        print(
            f"{YELLOW} -- Failed to update / check for updates for the submodule (remove the corresponding folder in code/app to redownload if possible): {folder} -- {END}"
        )
