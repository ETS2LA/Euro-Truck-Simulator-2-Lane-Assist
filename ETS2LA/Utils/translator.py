...
def get_available_languages(localedir: str) -> list:
    ...
    for lang in os.listdir(localedir):
        if os.path.isdir(os.path.join(localedir, lang, "LC_MESSAGES")):
            languages.append(Language.get(lang))
    return languages
...
class Translate:
    ...
    def ngettext(self, singular: str, plural: str, n: int) -> str:
        ...
        """
        return self.cleanup(self.translation.ngettext(singular, plural, n))
...
def set_language(language: str | Language):
    ...
    language = (
        Language.find(language) if not isinstance(language, Language) else language
    )
...
def generate_translations():
    ...
    os.system(
        f"python ETS2LA/Assets/gettext/pygettext.py -d ets2la -w 9999 -c TRANSLATORS -o {target_dir}/base.pot ."
    )
