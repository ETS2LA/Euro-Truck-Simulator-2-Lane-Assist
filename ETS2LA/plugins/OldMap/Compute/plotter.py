from ETS2LA.plugins.OldMap.GameData.prefabItems import PrefabItem, GetPrefabItemByUid
...
def map_curvature_to_max_speed(curvature):
...
def HandleNoNav(data, MapUtils, Enabled, closestData):
    ...
    for i in range(1, len(allPoints)):
        ...
        if np.array_equal(vector1, lastVector) or (angle < 80 and angle > -80):
            distance = math.sqrt((allPoints[i][0] - lastPoint[0]) ** 2 + (allPoints[i][1] - lastPoint[1]) ** 2)
            if distance > 2:
