...
def GetAIProperties():
    ...
    MODEL_TRAINING_DATE = " - - - "
    model = None
    for file in os.listdir(f"{variables.PATH}cache/TrafficLightDetection"):
        if file.endswith(".pt"):
            model = file
            break
    if model == None:
    ...
    MODEL_METADATA = {"data": []}
    try:
        torch.jit.load(os.path.join(f"{variables.PATH}cache/TrafficLightDetection", model), _extra_files=MODEL_METADATA)
    except:
         return MODEL_EPOCHS, MODEL_BATCH_SIZE, IMG_WIDTH, IMG_HEIGHT, MODEL_IMAGE_COUNT, MODEL_TRAINING_TIME, MODEL_TRAINING_DATE
    ...
    try:
        def LoadAIModelThread():
            try:
                ...
                try:
                    AIModel = torch.jit.load(os.path.join(f"{variables.PATH}cache/TrafficLightDetection", GetAIModelName()), map_location=AIDevice)
                    AIModel.eval()
                except:
            ...
            try:
                ...
                if response == 200:
                    ...
                    if str(LatestAIModel) != str(CurrentAIModel):
                        ...
                        last_progress = 0
                        with open(os.path.join(f"{variables.PATH}cache/TrafficLightDetection", f"{LatestAIModel}"), "wb") as modelfile:
...
def DeleteAllAIModels():
    try:
        ...
        for file in os.listdir(f"{variables.PATH}cache/TrafficLightDetection"):
            if file.endswith(".pt"):
                os.remove(os.path.join(f"{variables.PATH}cache/TrafficLightDetection", file))
...
def GetAIModelProperties():
    ...
    try:
        ...
        if GetAIModelName() == "UNKNOWN":
            return
        torch.jit.load(os.path.join(f"{variables.PATH}cache/TrafficLightDetection", GetAIModelName()), _extra_files=MODEL_METADATA, map_location=AIDevice)
        MODEL_METADATA = str(MODEL_METADATA["data"]).replace('b"(', '').replace(')"', '').replace("'", "").split(", ")
        for var in MODEL_METADATA:
            if "image_width" in var:
                IMG_WIDTH = int(var.split("#")[1])
