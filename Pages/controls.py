...
from ETS2LA.Handlers.controls import (
    get_event_information_dictionary,
    edit_event,
    unbind_event,
)
...
class Page(ETS2LAPage):
    ...
    def handle_change(self, alias: str):
        SendPopup(
            _("Please press the key / button you want to bind to this event."),
            type="info",
        )
        edit_event(alias)
    ...
    def render(self):
        ...
        with Container(styles.FlexHorizontal() + styles.Classname("justify-between")):
            with Container(styles.FlexVertical() + styles.Gap("12px")):
                ...
                Text(
                    _("You can change plugins' control events here."),
                    styles.Description(),
                )
        ...
        for control in controls:
            ...
            if self.target_control != "":
                has_in_name = (
                    self.target_control
                    and self.target_control.lower() in control.name.lower()
                )
                has_in_plugin = (
                    self.target_control
        ...
        if len(valid_controls) == 0:
            with Alert(style=styles.Padding("14px")):
                with Container(
                    ...
                    Text(
                        _("No controls found, try again with different filters."),
        ...
        for control in valid_controls:
            ...
            with Container(styles.FlexHorizontal() + styles.Gap("0px")):
                with Container(
                    ...
                    + left_style
                    + styles.Gap("8px")
                    + styles.Classname("w-full rounded-md border p-4 bg-input/10")
                ):
                    with Container(
                        styles.FlexHorizontal()
                        + styles.Gap("24px")
                        ...
                        Separator(
                            direction="vertical",
                            style=styles.Width("1px") + styles.Height("100%"),
                        )
                ...
                with Container(
                    ...
                    + right_style
                    + styles.Classname(
                        "rounded-md border justify-center pl-2 pr-2 bg-input/10"
                    )
                ):
                    with Button(
                        action=self.handle_change,
                        name=control.alias,
                        style=styles.Width("28px")
                        + styles.Height("28px")
