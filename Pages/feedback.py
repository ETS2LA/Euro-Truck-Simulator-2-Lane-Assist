from ETS2LA.Networking.cloud import GetUsername, SendFeedback
...
class Page(ETS2LAPage):
    ...
    def send_feedback(self):
        ...
        if self.want_response and self.username == "":
            SendPopup("Please provide your Discord username or email address if you would like a response.", "error")
            return
        
        fields = {}
        if self.want_response:
            fields["Wants response"] = "Yes"
            fields["Response contact"] = self.username if self.username != "" else "None given, ignore the request."
        ...
        if success:
            ...
            SendPopup("Could not send feedback. Please check your internet connection and try again later.", "error")
    
    def render(self):
        with Container(styles.Style(
            padding="20px",
        )):
            ...
            with Container(styles.FlexHorizontal() + styles.Classname("align_center items-center") + styles.Height("50px")):
                with Container(styles.FlexHorizontal()):
                    ...
                    Text("I would like a response", styles.Style(margin_left="10px", min_width="150px"))
                if self.want_response:
                    Input(
            ...
            with Container(styles.FlexHorizontal() + styles.Classname("align_center items-center") + styles.Height("50px")):
                ...
                with Container(styles.Classname("border rounded-md p-2 w-full " + ("bg-input/10" if not self.share_location else "bg-input/30"))):
                    ...
                    text += ", " + f"{data.get('truckPlacement', {}).get('coordinateY'):.0f}"
                    text += ", " + f"{data.get('truckPlacement', {}).get('coordinateZ'):.0f}"
                    Text(text, styles.Classname("text-muted-foreground") if not self.share_location else styles.Style())
