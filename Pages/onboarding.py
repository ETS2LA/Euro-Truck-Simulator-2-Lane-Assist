...
class Page(ETS2LAPage):
    ...
    def handle_skip_onboarding(self):
        SendPopup(
            _(
                "If you've done the onboarding before, you know how to exit. ETS2LA will not work without doing the onboarding once."
            )
        )
    ...
    def loading(self):
        with Container(
            ...
            + styles.Classname("items-center relative justify-center gap-4 text-center")
            + styles.Height("100vh")
        ):
            Text(_("Loading..."), styles.Title())
            Text(
                _(
                    "We are doing something that is taking a while. Just sit tight and relax!"
                ),
                styles.Description(),
            )
    ...
    def welcome(self):
        with Container(style=styles.FlexVertical() + styles.Width("400px")):
            ...
            Text(
                _(
                    "Let's get you setup with the basics right off the bat! Press the button below to continue."
                ),
                styles.Description(),
            )
        ...
        with Container(
            style=styles.Classname("bottom-2 absolute left-0 right-0 text-center")
        ):
        ...
        with Container(
            style=styles.Classname(
                "bottom-0 absolute right-0 text-center flex gap-2 items-center"
            )
            + offset
        ):
            ...
            Text(
                _("You can change the theme here!"),
                styles.Description()
    ...
    def language(self):
        with Container(
            ...
            with Container(style=styles.FlexVertical() + styles.Width("400px")):
                ...
                Text(
                    _(
                        "Please select your preferred language for ETS2LA. This option can be changed later in the options. All our translations are community made!"
                    ),
                    styles.Description(),
                )
                Space()
                with Button(
                    action=self.increment_page, style=styles.Classname("default w-full")
                ):
                    Text(_("Continue"))
            ...
            with Container(style=styles.FlexVertical()):
                ...
                with Alert(
                    style=styles.Padding("14px")
                    + styles.Width("400px")
                    + styles.Classname("default")
                ):
                    with Container(
                        ...
                        with Container(styles.FlexVertical() + styles.Gap("4px")):
                            Text(
                                f"This translation is {_.get_percentage():.2f}% complete.",
    ...
    def sdk_setup(self):
        with Container(
            ...
            with Container(style=styles.FlexVertical() + styles.Width("470px")):
                ...
                Text(
                    _(
                        "To get started, we need to set up the SDK. This will allow ETS2LA to interact with your game. To the right you will find a list of games, click install on one of them and then continue."
                    ),
                    styles.Description(),
                )
                Space()
                if not sdk_page.CanContinue():
                    Text(
                        _("Waiting for SDK installation..."),
                        styles.Description() + styles.Classname("text-xs"),
                    )
                else:
                    with Button(action=self.increment_page):
    ...
    def plugins(self):
        with Container(
            ...
            with Container(style=styles.FlexVertical() + styles.Width("450px")):
                ...
                Text(
                    _(
                        "ETS2LA uses plugins to provide most of it's functionality. You can select between a simple mode, and an advanced mode. This can be changed later on in the settings."
                    ),
                    styles.Description() + styles.MaxWidth("520px"),
                )
                Space()
                with Button(
                    action=self.increment_page, style=styles.Classname("default w-full")
            ...
            with Container(
                style=styles.Classname("text-start")
                + styles.Width("550px")
                + styles.FlexVertical()
                + styles.Gap("12px")
            ):
                ...
                with Alert(
                    ...
                    with Container(
                        ...
                        if not settings.advanced_plugin_mode:
                            ...
                            Markdown(
                                _(
                                    "Keeping **Advanced Plugin Mode** disabled is recommended, however advanced users who want to select plugins themselves might want it enabled."
                                ),
                                styles.Classname("text-muted-foreground"),
                            )
                        else:
                            ...
                            Markdown(
                                _(
                                    "**Advanced Plugin Mode** unlocks manual control of enabled plugins. It also lets you download more plugins from other repositories. Keep in mind that these 3rd party plugins are not *officially* supported by ETS2LA."
                                ),
                                styles.Classname("text-muted-foreground"),
                            )
    ...
    def priority(self):
        with Container(
            ...
            with Container(style=styles.FlexVertical() + styles.Width("450px")):
                ...
                Text(
                    _(
                        "ETS2LA can automatically tell your operating system that it should run in a high priority mode. This is recommended especially on lower end devices."
                    ),
                    styles.Description() + styles.MaxWidth("520px"),
                )
                Space()
                with Button(
                    action=self.loading_increment,
                    style=styles.Classname("default w-full"),
                ):
                    Text(_("Continue"))
                ...
                if not state:
                    with Alert(
                        ...
                        with Container(
                            ...
                            Markdown(
                                _(
                                    "You'll need to restart ETS2LA after doing the onboarding to apply this change."
                                ),
                                styles.Classname("text-muted-foreground"),
                            )
    ...
    def map_data(self):
        with Container(
            ...
            with Container(style=styles.FlexVertical() + styles.Width("450px")):
                ...
                Text(
                    _(
                        "ETS2LA uses pre-extracted map data from the game. We handle the data beforehand and you can then just download it as necessary. Please select the data you need for your current game version below."
                    ),
                    styles.Description(),
                )
                Space()
            ...
            with Container(style=styles.FlexVertical() + styles.Width("600px")):
                ...
                if current:
                    ...
                    with Container(
                        ...
                        Text(
                            _(
                                "Please select the data you want to use from the dropdown above."
                            ),
                            styles.Description() + styles.Classname("text-xs"),
                        )
    ...
    def size(self):
        with Container(
            ...
            with Container(style=styles.FlexVertical() + styles.Width("450px")):
                ...
                Text(
                    _(
                        "ETS2LA bypasses Windows to provide custom window features. Due to this you cannot resize the window normally. These are sliders to manually change the window size."
                    ),
                    styles.Description(),
                )
                Space()
    ...
    def window_controls(self):
        with Container(
            ...
            with Container(style=styles.FlexVertical() + styles.Width("450px")):
                ...
                Text(
                    _(
                        "ETS2LA provides custom window controls that might be useful to you. These can be accessed through the green button in the top right corner."
                    ),
                    styles.Description(),
                )
                Space()
            ...
            with Container(
                style=styles.Classname(
                    "top-0 absolute right-0 text-center flex gap-2 items-center"
                )
                + offset
            ):
                Text(
                    _("It's this green button!"),
                    styles.Description()
                    + styles.Classname("text-xs text-muted-foreground"),
                )
                icon_style = styles.Style()
    ...
    def complete(self):
        with Container(style=styles.FlexVertical() + styles.Width("400px")):
            ...
            Text(
                _(
                    "You have successfully completed the onboarding process. You can now start using ETS2LA. If you need help, feel free to join our Discord server."
                ),
                styles.Description() + styles.MaxWidth("520px"),
            )
            Space()
        ...
        if self.show_sidebar:
            ...
            with Container(
                style=styles.FlexVertical()
                + styles.Classname("left-2 top-0 absolute items-center justify-center")
                + manager_button_offset
            ):
    ...
    def render(self):
        ...
        with Container(
            ...
            if self.page in pages:
                with Container(
                    style=styles.FlexVertical()
                    + styles.Classname("justify-center gap-4 text-left")
                    + styles.Height("100vh")
                ):
                    with Button(
                        action=self.decrement_page,
                        style=styles.Classname("bg-transparent w-max p-0 h-max"),
                        type="ghost",
                    ):
                        Text(
                            f"0{self.page + 1} / 0{len(pages)}",
                            styles.Classname("text-xs text-muted-foreground"),
                        )
                    pages[self.page]()
            else:
