...
class Page(ETS2LAPage):
    ...
    def priority(self):
        with Container(style=styles.FlexHorizontal() + styles.Width("1000px") + styles.Gap("48px")):
            with Container(style=styles.FlexVertical() + styles.Width("450px")):
                Text("High Priority Mode", styles.Title())
                Text("ETS2LA can automatically tell your operating system that it should run in a high priority mode. This is recommended especially on lower end devices.", styles.Description() + styles.MaxWidth("520px"))
                Space()
                ...
                if not state:
                    with Alert(style=styles.FlexVertical() + styles.Padding("14px") + styles.Classname("default") + styles.Gap("6px")):
                        with Container(styles.FlexHorizontal() + styles.Gap("12px") + styles.Classname("items-start")):
                            ...
                            style.color = "var(--muted-foreground)"
                            Icon("info", style)
                            Markdown("You'll need to restart ETS2LA after doing the onboarding to apply this change.", styles.Classname("text-muted-foreground"))
