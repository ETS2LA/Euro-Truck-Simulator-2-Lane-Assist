...
class BasicModeFeature:
    ...
    def __init__(
        self, name: str, description: str, plugin_ids: list[str], default: bool = True
    ):
        self.name = name
...
basic_mode_features = [
    BasicModeFeature(
        name=_("Steering"),
        description=_(
            "Enable automated steering using ETS2LA. You can toggle it on and off using the 'Toggle Steering' keybind in the control settings."
        ),
    ...
    BasicModeFeature(
        ...
        description=_(
            "Enable automated acceleration and deceleration using ETS2LA. You can toggle it on and off using the 'Toggle Speed Control' keybind in the control settings."
        ),
...
class Page(ETS2LAPage):
    ...
    # MARK: Basic mode
    feature_toggles = {feature.name: feature.default for feature in basic_mode_features}
    ...
    def enable_plugin(self, plugin_name: str):
        ...
        threading.Thread(
            target=plugins.start_plugin, kwargs={"folder": plugin_name}, daemon=True
        ).start()
    ...
    def disable_plugin(self, plugin_name: str):
        ...
        threading.Thread(
            target=plugins.stop_plugin, kwargs={"folder": plugin_name}, daemon=True
        ).start()
    ...
    def render_plugin(self, plugin):
        ...
        with Button(
            ...
            + styles.Width("100%")
            + (
                styles.Classname("bg-input/20")
                if plugin.running
                else styles.Classname("bg-input/10")
            ),
        ):
            with Container(
                ...
                with Container(style=styles.FlexHorizontal() + styles.Gap("15px")):
                    if plugin.folder in self.enabling:
                        with Spinner():
                            Icon("loader")
                    with Container(style=styles.FlexVertical() + styles.Gap("5px")):
    ...
    def render_plugin_list(self, plugins_list):
        ...
        while i < len(plugins_list):
            with Container(
                styles.FlexHorizontal() + styles.Gap("20px") + styles.Height("100%")
            ):
                if i + 1 == len(plugins_list):
                    ...
                    with Container(styles.Width("50%") + styles.Height("auto")):
                        ...  # Fake second item to fill the space
                else:
                    for j in range(2):
        ...
        if isBasic:
            with Container(
                ...
                with Container(
                    ...
                    Text(
                        _(
                            "You can enable different ETS2LA features on the right side. All of the plugins described by these features will be enabled automatically without any additional configuration."
                        ),
                        styles.Description(),
                    )
                    with Button(self.toggle_running, enabled=not self.update_queue):
                    ...
                    for feature in basic_mode_features:
                        with Button(
                            ...
                            + styles.Classname(
                                ...
                                + (
                                    "bg-background"
                                    if not self.feature_toggles[feature.name]
                                    else ""
                                )
                            ),
                            ...
                            with Container(
                                styles.FlexVertical()
                                + styles.Gap("5px")
                                + styles.Classname("text-left w-full")
                            ):
                                with Container(
            ...
            with Container(
                ...
                + styles.Padding("20px 20px 0 20px")
            ):
                ...
                with Container(
                    ...
                    Combobox(
                        ...
                        search=ComboboxSearch(
                            placeholder=_("Search tags..."), empty=_("No tags found")
                        ),
                    )
                    Combobox(
                        options=authors,
            ...
            running_plugins = [
                plugin
                for plugin in plugins.plugins
                if plugin.running and plugin.folder not in self.disabling
            ]
            running_plugins += [
                plugin
                for plugin in plugins.plugins
            ...
            filtered_plugins = [
                plugin for plugin in plugins.plugins if plugin not in running_plugins
            ]
            ...
            filtered_plugins = [
                ...
                if (
                    not self.authors
                    or (
                        isinstance(plugin.authors, list)
                        and any(
                            author.name.lower() in self.authors
                            for author in plugin.authors
                        )
                    )
                    or (
                        not isinstance(plugin.authors, list)
                        and plugin.authors.name.lower() in self.authors
                    )
                )
            ]
            # sort by name
            ...
            with Container(
                ...
                if not running_plugins:
                    if not last_plugins:
                        with Alert():
                            Text(
                                _("No plugins are currently running."),
                                styles.Description(),
                            )
                    else:
                        with Alert(style=styles.Gap("20px") + styles.FlexVertical()):
                            Markdown(
                                f"{_('Want to enable the plugins you had running last time?')}\n\n**{', '.join(last_plugins)}**",
                                styles.Description(),
                            )
            ...
            with Container(
                styles.FlexVertical()
                + styles.Gap("20px")
                + styles.Padding("0px 20px 20px 20px")
            ):
