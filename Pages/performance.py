...
class PerformanceMetrics:
    ...
    def cpu_thread(self):
        ...
        while True:
            ...
            self.output.put_nowait({"cpu": round(val, 1)})
    ...
    def ram_thread(self):
        ...
        while True:
            ...
            self.output.put_nowait(
                {"ram": last_ram, "ets2la_ram": round(last_ets2la_ram, 1)}
            )
...
class Page(ETS2LAPage):
    ...
    def format_frametimes_to_graph_data(self, frametimes: list[float]):
        ...
        for i, frametime in enumerate(frametimes):
            fps = 1000 / frametime if frametime > 0 else 0
            graph_data.append({"time": i, "fps": fps})
    ...
    def render(self):
        ...
        with Container(styles.FlexVertical() + styles.Classname("p-4")):
            with Tabs():
                with Tab(_("System")):
                    if len(self.cpu_usage) < 60 or len(self.ram_usage) < 60:
                        with Container(
                            styles.FlexVertical()
                            + styles.Classname("border rounded-md p-4 w-full")
                        ):
                            Text(
                                _(
                                    "Data is still being collected, please wait a few seconds for the graphs to stabilize."
                                ),
                                styles.Description(),
                            )
                    ...
                    with Container(styles.FlexHorizontal() + styles.Classname("gap-6")):
                        with Container(
                            styles.FlexVertical()
                            + styles.Classname(
                                "border rounded-md p-4 w-full h-full relative"
                            )
                            + styles.Height("320px")
                        ):
                            with Container(
                                styles.FlexHorizontal() + styles.Classname("z-10 w-max")
                            ):
                                Text(_("CPU Usage"))
                                if self.cpu_usage:
                                    Text(
                                        f"{round(self.cpu_usage[-1])}%",
                                        styles.Description(),
                                    )

                            with Container(
                                styles.Classname("absolute bottom-0 left-0 right-0")
                            ):
                                Graph(
                                    data=[
                                        {"time": i, "cpu": value}
                                        for i, value in enumerate(self.cpu_usage)
                                    ],
                        ...
                        with Container(
                            styles.FlexVertical()
                            + styles.Classname(
                                "relative border rounded-md p-4 w-full h-full"
                            )
                            + styles.Height("320px")
                        ):
                            with Container(
                                    ...
                                    data=[
                                        {
                                            "time": i,
                                            "ram": value,
                                            "ets2la_ram": self.ets2la_mem_usage[i]
                                            if len(self.ets2la_mem_usage) > i
                                            else 0,
                                        }
                                        for i, value in enumerate(self.ram_usage)
                                    ],
                                    config={
                                        "ram": {"label": _("RAM Usage ")},
                                        "ets2la_ram": {
                                            "label": _("ETS2LA RAM Usage  ")
                                        },
                                    },
                                    x=GraphAxisOptions("time"),
                                    y=[
                                        GraphAxisOptions("ram", max=100, min=0),
                                        GraphAxisOptions(
                                            "ets2la_ram",
                                            max=100,
                                            min=0,
                ...
                with Tab(_("Plugins")):
                    with Container(styles.FlexVertical() + styles.Classname("gap-6")):
                        running_plugins = [
                            plugin for plugin in plugins.plugins if plugin.running
                        ]
                        if not running_plugins:
                            Text(
                                _("No plugins are currently running."),
                                styles.Description() + styles.Classname("font-bold"),
                            )
                        ...
                        for plugin in running_plugins:
                            try:
                                ...
                                if plugin.description.id not in self.first_times:
                                    self.first_times[plugin.description.id] = (
                                        plugin.frametimes[0]
                                    )

                                with Container(
                                    styles.FlexVertical()
                                    + styles.Classname("relative border rounded-md p-4")
                                    + styles.Height("200px")
                                ):
                                    with Container(
                                        styles.FlexHorizontal()
                                        + styles.Classname("z-10 w-max")
                                    ):
                                        ...
                                        with Tooltip() as t:
                                            with t.trigger as tr:
                                                ...
                                                Text(
                                                    self.format_frametime(
                                                        plugin.frametimes[-1],
                                                        plugin.description.fps_cap,
                                                    ),
                                                    styles.Description(),
                                                )
                                            with t.content:
                                                try:
                                                    average = (
                                                        sum(plugin.frametimes)
                                                        / len(plugin.frametimes)
                                                        if plugin.frametimes
                                                        else 0
                                                    )
                                                    maximum = (
                                                        max(plugin.frametimes)
                                                        if plugin.frametimes
                                                        else 0
                                                    )
                                                    minimum = (
                                                        min(plugin.frametimes)
                                                        if plugin.frametimes
                                                        else 0
                                                    )
                                                    ...
                                                    Text(
                                                        f"Avg: {round(1000 / average, 2) if average > 0 else 0} FPS",
                                                        styles.Description(),
                                                    )
                                                    Text(
                                                        f"Min: {round(1000 / maximum, 2) if maximum > 0 else 0} FPS",
                                                        styles.Description(),
                                                    )
                                                    Text(
                                                        f"Max: {round(1000 / minimum, 2) if minimum > 0 else 0} FPS",
                                                        styles.Description(),
                                                    )
                                    ...
                                    if (
                                        ...
                                        Text(
                                            _(
                                                "Warning: Graph is still gathering data, please wait 60 seconds for it to stabilize."
                                            ),
                                            styles.Description()
                                            + styles.Classname("text-xs"),
                                        )

                                    graph_data = self.format_frametimes_to_graph_data(
                                        plugin.frametimes
                                    )
                                    graph_config = {
                                        "fps": {
                                    ...
                                    with Container(
                                        ...
                                        Graph(
                                            ...
                                            y=GraphAxisOptions(
                                                "fps",
                                                max=plugin.description.fps_cap,
                                                min=0,
                                            ),
                                            type="area",
                                ...
                                Text(
                                    _(
                                        "Failed to render plugin {plugin_name}.",
            ...
            with Container(
                styles.FlexVertical() + styles.Classname("border rounded-md p-4 w-full")
            ):
                Text(
                    _("All displayed data is averaged over a second."),
                    styles.Description(),
                )
                Text(
                    _("This page took {time:.2f} ms to render.").format(
                        time=(time.perf_counter() - start_time) * 1000
                    ),
                    styles.Description(),
                )
