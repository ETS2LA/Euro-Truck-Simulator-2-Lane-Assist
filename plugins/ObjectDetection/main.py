...
class Plugin(ETS2LAPlugin):
    ...
    def imports(self):
        # Import libraries
        global Detection, Tracker, OptimizedKalmanFilterFactory
        from norfair import Detection, Tracker, OptimizedKalmanFilterFactory
        
        global Literal, tqdm, np, screeninfo, threading, warnings, requests, pathlib, torch, time, math, cv2, os
        ...
        try:
            global ScreenLine, Text
            from plugins.AR.main import ScreenLine, Text
        except ImportError:
            pass

    def init(self):
        settings_yolo_fps = settings.Get("ObjectDetection", "yolo_fps", 2)
        global MODE, YOLO_FPS, MODEL_TYPE, MODEL_NAME, MODELS_DIR, MODEL_PATH, MODEL_REPO, MODEL_DOWNLOAD_LINK, USE_EXTERNAL_VISUALIZATION, TRACK_SPEED
        MODE = settings.Get("ObjectDetection", "mode", "Performance")
        YOLO_FPS = settings_yolo_fps if settings_yolo_fps > 0 else 2
        ...
        TRACK_SPEED = ["car", "van", "bus", "truck"]
        
        global model_download_html, connection_failed_html
    ...
    def Initialize(self):
        ...
        ScreenCapture = self.modules.ScreenCapture
        PositionEstimation = self.modules.PositionEstimation
        ScreenCapture.mode = "grab"
        Raycast = self.modules.Raycasting
        ...
        if not os.path.exists(MODEL_PATH):
            ...
            choice = self.ask(model_download_html, [Translate("cancel"), Translate("download")])
            translated_downloading = Translate("downloading", [MODEL_NAME])
            if choice == Translate("download"):
                self.state = translated_downloading
    ...
    def run(self):
        ...
        if MODE == "Performance":
            tracked_boxes = self.track_cars(boxes, yolo_frame)
