...
class UI():
    try: # The panel is in a try loop so that the logger can log errors if they occur
        ...
        def exampleFunction(self):
            ...
            helpers.MakeLabel(self.root, "About", 0,0, font=("Roboto", 20, "bold"), padx=30, pady=10, columnspan=2)
            helpers.MakeLabel(self.root, "The app backend was made by Tumppi066. In addition to the following plugin developers:", 1,0, font=("Roboto", 10), padx=30, pady=10, columnspan=2)
            developers = self.findDevelopers()
            helpers.MakeLabel(self.root, ", ".join(developers), 2,0, font=("Roboto", 8), padx=30, pady=2, columnspan=2)
            helpers.MakeLabel(self.root, " ", 3,0, font=("Roboto", 10), padx=30, pady=10, columnspan=2)
            helpers.MakeLabel(self.root, "To all those who supported me:", 4,0, font=("Roboto", 10), padx=30, pady=10, columnspan=2)
            helpers.MakeLabel(self.root, "Thank you from the bottom of my heart!", 5,0, font=("Roboto", 20, "bold"), padx=30, pady=0, columnspan=2)
            helpers.MakeLabel(self.root, " ", 6,0, padx=30, pady=2, columnspan=2)
            ...
            helpers.MakeLabel(self.root, "all the models I used in the default plugins to pytorch and onnx.", 8,0, padx=30, pady=2, columnspan=2)
            helpers.MakeLabel(self.root, "And of course the researchers who made those models in the first place!", 9,0, padx=30, pady=2, columnspan=2)
            helpers.MakeLabel(self.root, " ", 10,0, padx=30, pady=2, columnspan=2)
            helpers.MakeLabel(self.root, "I'm also going to give a shoutout to the developers of sv_ttk and dxcam. They have been essential ", 11,0, padx=30, pady=2, columnspan=2)
