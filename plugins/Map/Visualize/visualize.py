...
def VisualizePrefabs(data, img=None, zoom=2):
    ...
    for item in areaItems:
        if item.Prefab.ValidRoad:
            ...
            cv2.putText(img, filename, (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            
            for curve in item.Prefab.PrefabCurves:
                curveCount += 1
                startXY = prefabItems.GetLocalCoordinateInTile(curve.startX + item.X, curve.startZ + item.Z, tileCoords[0], tileCoords[1])
                endXY = prefabItems.GetLocalCoordinateInTile(curve.endX + item.X, curve.endZ + item.Z, tileCoords[0], tileCoords[1])
