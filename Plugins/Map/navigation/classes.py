...
class RouteNode:
    ...
    def calculate_lanes_from(self, last, direction: Literal["forward", "backward"]):
        ...
        nav_info = (
            navigation_information.forward
            if direction == "forward"
            else navigation_information.backward
        )
        ...
        if self.item is None:
            ...
            logging.warning(
                f"Item is None: Calculated {len(lanes)} valid lanes for node {self.node.uid} ({self.direction})"
            )
            return
        ...
        if last.item is None:
            ...
            self.is_possible = len(lanes) > 0
            logging.warning(
                f"Last Item is None: Calculated {len(lanes)} valid lanes for node {self.node.uid} and item {item.uid} ({type(item).__name__}) ({self.direction})"
            )
            return
        ...
        for _last_index, last_lane in last_lanes:
            ...
            for cur_index, cur_lane in cur_lanes:
                ...
                ss_distance = mh.DistanceBetweenPoints(
                    last_start_point, cur_start_point
                )
