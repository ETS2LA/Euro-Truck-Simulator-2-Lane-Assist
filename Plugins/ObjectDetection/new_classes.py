...
class Vehicle:
    ...
    def from_old_class(self, old_class):
        left = old_class.raycasts[0].point
        right = old_class.raycasts[1].point
        average = [(left[0] + right[0]) / 2, (left[1] + right[1]) / 2, (left[2] + right[2]) / 2]
        self.position = Position(*average)
        
        size = math.sqrt((left[0] - right[0]) ** 2 + (left[1] - right[1]) ** 2 + (left[2] - right[2]) ** 2)
        self.size = Size(size, size / 1.5, size * 2)
        
        self.speed = old_class.speed
        self.id = old_class.id
