...
class Widget(HUDWidget):
    ...
    def draw(self, offset_x, width, height=50):
        ...
        for i in range(num_lines):
            ...
            data.append(
                Line(
                    Point(50 + offset_x + lane_offset, height-8 + v_offset, anchor=self.plugin.anchor),
                    Point(width-60 + offset_x + lane_offset, 8 + v_offset, anchor=self.plugin.anchor),
                    color=line_color,
                    thickness=thickness
                )
            )
            ...
            data.append(
                Line(
                    Point(width-20 + offset_x + lane_offset, height-8 + v_offset, anchor=self.plugin.anchor),
                    Point(90 + offset_x + lane_offset, 8 + v_offset, anchor=self.plugin.anchor),
                    color=line_color,
                    thickness=thickness
                )
            )
