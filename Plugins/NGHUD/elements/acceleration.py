    ...
    def draw(self, offset_x, width, height=50):
        ...
        if acc:
            ...
            if not target_acceleration:
                ...
                target_acceleration = self.plugin.globals.tags.merge(target_acceleration)
        ...
        if not acc:
            ...
            self.data = [
                ...
                Text(
                    ...
                    text=f"{target_acceleration:.1f} target" if target_acceleration else "-.-",
                    color=Color(255, 255, 255, 200),
                    size=14
                ),
                Text(
                    Point(width-32 + offset_x, height-20, anchor=self.plugin.anchor),
                    text=f"m/s²",
                    color=Color(255, 255, 255, 200),
                    size=14
                )
