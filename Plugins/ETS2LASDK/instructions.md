## Installation
Navigate to this folder in your terminal. Then write the following:
```bash
dotnet new --install .
```
Now if you run:
```bash
dotnet new list
```
You should see `ets2la-plugin` in the list of installed templates. You should now refresh your VS Code window to make the template available.

## Usage
Press F1 in VS Code and type `> .NET: New Project...`, then select `ETS2LA Plugin` from the list of templates. Follow the instructions to create your new plugin project.