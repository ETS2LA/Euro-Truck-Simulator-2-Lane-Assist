...
class Widget(HUDWidget):
    name = _("Closest City")
    description = _("Displays the closest city and its distance ( relative to the text ).")
    fps = 2
    ...
    def draw(self, offset_x, width, height=50):
        ...
        if closest_city_distance:
            closest_city_distance = self.plugin.globals.tags.merge(closest_city_distance)
        else:
